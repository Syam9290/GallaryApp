...
class AppStore extends Component {
  state = {appName: '', tabItem: tabsList[0].tabId}
  ...
  render() {
    const {appName, tabItem} = this.state

    const filterTabItems = appsList.filter(
      eachObj => eachObj.category === tabItem,
    )

    const filterSearchApps = filterTabItems.filter(eachObj =>
      eachObj.appName.toLowerCase().includes(appName.toLowerCase()),
    )
    return (
      <div className="bgContainer">
        ...
        <ul className="tabItems">
          {tabsList.map(eachObj => (
            <TabItem
              tabItem={eachObj}
              getTabItems={this.getTabItems}
              isActiveTab={eachObj.tabId === tabItem}
              key={eachObj.tabId}
            />
          ))}
        </ul>
        <ul className="appItems">
          {filterSearchApps.map(eachObj => (
            <AppItem items={eachObj} key={eachObj.appId} />
          ))}
        </ul>
      </div>
    )
